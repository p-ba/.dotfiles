...
run_case("mixed-html/php open-block indentation", {
  "<div>",
  "<?php",
  "if ($x) {",
  "$y = 1;",
...
run_case("mixed-html/php after opening tag", {
  "<div>",
...
run_case("case 4: enter after array element with comma and trailing // comment", {
...
run_case("case 4: enter after foreach body statement with ; and trailing // comment", {
  '<div class="line big mx-auto"></div>',
  "<?php",
...
run_case("case 4: closing ]; aligns with opening $var = [", {
  '<div class="line big mx-auto"></div>',
  "<?php",
  "$cats = [",
  "    'taxonomy' => 'category',",
...
run_buffer_case("case 5c: closing ], and ]; with trailing // comments dedent", {
  "$more_posts_params = [",
  "\t'nested_array' => [",
  "\t\t'wat' => 'lol',",
  "\t], //incorrect",
...
run_buffer_case("case 5b: closing ] / ], dedents even in pure php file", {
  "$x = [",
  "\t'nested' => [",
  "\t\t'k' => 'v',",
  "\t],",
...
run_buffer_case("nested: closing ] aligns with key => [ (even if treesitter returns 0)", {
  "$cats = [",
  "\t'' => [",
  "\t\t'',",
...
run_buffer_case("php-only: method call array close `]);` + following return view array `]);`", {
  "<?php",
  "$payment->update([",
  "    'request_payload' => [",
...
run_buffer_case("php-only: multiline call arg array `],` + closing `);` align with openers", {
  "<?php",
...
run_buffer_case("php-only: multiline if conditions closing `) {` aligns with `if (`", {
  "<?php",
  "if (",
...
run_buffer_case("php-only: multiline logical operator lines do not staircase", {
  "<?php",
  "if (",
  "    ! Schema::hasTable('ups_credentials')",
  "        || ! Schema::hasTable('shipping_countries')",
...
run_buffer_case("php-only: multiline ternary `?` and `:` align under condition line", {
  "<?php",
...
run_buffer_case("php-only: statement after multiline ternary dedents", {
  "<?php",
  "$targetCredentialId = in_array($iso2, self::EU_COUNTRY_ISO2, true)",
  "    ? (int) $euCredential->getKey()",
  "    : (int) $rowCredential->getKey();",
...
run_buffer_case("php-only: multiline chained method calls keep `->` indentation level", {
  "<?php",
...
run_buffer_case("php-only: line after chained method terminator dedents to chain base", {
  "<?php",
